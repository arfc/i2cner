...
\tikzstyle{const} = [rectangle, draw, text centered, fill=purple!20]
...
\begin{figure}
	...
	\scalebox{1.0}{
		...
		\node [cloud, below=\figwidth of times] (mod) {\texttt{MODEL} \\ heterogeneous \\ multi-technology \\ model of Japan};
                \node [cloud, above left=1.5\figwidth and 2*\figwidth of times] (dat) {\texttt{DATA}\\regarding\\both\\i$^2$cner\\ \&\\ conventional\\technologies};
		...
		\draw[->, ultra thick] (dat13) -- (dat);
               % \draw[out=330,in=300,looseness=8] (mod) -- (mod);
        \end{scope}
 \end{tikzpicture}
	}
